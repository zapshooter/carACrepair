...
<?php
  ...
  if(isset($_POST['sub']))
  {
    ...
    foreach($arrayData as $field) {
      if (empty($field)) {
        echo "<script>location.href='panel.php?page=master-department&e=b'</script>";die;
      }
    }
    if(checkExists($table, $arrayData) == 1){
      echo "<script>location.href='panel.php?page=master-department&e=e'</script>";die;
    }
    $arrayData['status'] = filter_values($_POST['status']);
    if(save($table, $arrayData) == 1)
    {
       echo "<script>location.href='panel.php?page=master-department&s=a'</script>";die;
    }
    else
    {
        echo "<script>location.href='panel.php?page=master-department&e=w'</script>";
    }
  }
  if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'delete')
  {
    ...
    if(isset($sno) && !empty($sno) && $sno != ''){
      ...
      {
        echo "<script>location.href='panel.php?page=master-department&s=d'</script>";die;
      }
      else
      {
        echo "<script>location.href='panel.php?page=master-department&e=w'</script>";
        ...
        <div class="row">
          <div class="col-12">
                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr>
                    ...
                    <td class="project-actions text-right">
                      <a class="btn btn-info btn-sm" href="panel.php?page=master-department&sno=<?php echo base64_encode($data['sno']);?>&action=edit">
                          <i class="fas fa-pencil-alt">
                          </i>
                          Edit
                      </a>
                      <a class="btn btn-danger btn-sm" onclick="return confirm('Are You Sure To Delete This Row.')" href="panel.php?page=master-department&sno=<?php echo base64_encode($data['sno']);?>&action=delete">
            ...
            <div class="card card-primary">
				<div class="card-header">
					<h3 class="card-title">Add New <small>Department</small></h3>
				</div>
              <!-- /.card-header -->
              <!-- form start -->
					...
					<div class="card-body">
						<div class="form-row">
							<div class="form-group col-md-6">
								...
								<input type="text" name="name" class="form-control" id="department" placeholder="Enter department">
            ...
            else if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'edit')
            {
              if(isset($_GET['sno']) && $_GET['sno']!='')
              {
                ...
                {
                  echo '<script>window.location.href="panel.php?page=master-department";</script>';die();
                }
              }
              else
              {
					...
					<div class="card-body">
						<div class="form-row">
							<div class="form-group col-md-6">
								...
								<input type="text" name="name" class="form-control" id="department" placeholder="Enter Department" value="<?php echo $name; ?>">
							</div>
							<div class="form-group col-md-6">
								<label for="status">Status</label>
								<select id="status" name="status" class="form-control custom-select">
