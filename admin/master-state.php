...
<?php
  ...
  if(isset($_POST['sub']))
  {
    ...
    foreach($arrayData as $field) {
      if (empty($field)) {
        echo "<script>location.href='panel.php?page=master-state&e=b'</script>";die;
      }
    }
    if(checkExists($table, $arrayData) == 1){
      echo "<script>location.href='panel.php?page=master-state&e=e'</script>";die;
    }
    $arrayData['status'] = filter_values($_POST['status']);
    if(save($table, $arrayData) == 1)
    {
       echo "<script>location.href='panel.php?page=master-state&s=a'</script>";die;
    }
    else
    {
        echo "<script>location.href='panel.php?page=master-state&e=w'</script>";
    }
  }
  if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'delete')
  {
    ...
    if(isset($sno) && !empty($sno) && $sno != ''){
      ...
      {
        echo "<script>location.href='panel.php?page=master-state&s=d'</script>";die;
      }
      else
      {
        echo "<script>location.href='panel.php?page=master-state&e=w'</script>";
          ...
          <div class="col-12">
            ...
            <div class="card">
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <?php
                  $s = 0;
                    $field_array = array('sno', 'name', 'country_id', 'status');
                    $where = "";
                    $orderby = "name ASC";
                  ...
                  <tr>
                    ...
                    <td><?php 
						...
						$country = retrieve('master_country', $c_id, $field_array, $where);
                    ...
                    <td class="project-actions text-right">
                      <a class="btn btn-info btn-sm" href="panel.php?page=master-state&sno=<?php echo base64_encode($data['sno']);?>&action=edit">
                          <i class="fas fa-pencil-alt">
                          </i>
                          Edit
                      </a>
                      <a class="btn btn-danger btn-sm" onclick="return confirm('Are You Sure To Delete This Row.')" href="panel.php?page=master-state&sno=<?php echo base64_encode($data['sno']);?>&action=delete">
						...
						<div class="form-row">
							<div class="form-group col-md-4">
								...
								<select name="country_name" class="form-control custom-select">
									...
									<?php 
										$field_array = array('sno', 'name');
										$where = "master_country.status = '1'";
										$order_by = "name ASC";
										$country_list = retrieve('master_country', null, $field_array, $where, $order_by);
							...
							<div class="form-group col-md-4">
								...
								<input type="text" name="name" class="form-control" id="country" placeholder="Enter State Name">
            ...
            else if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'edit')
            {
              if(isset($_GET['sno']) && $_GET['sno']!='')
              {
                ...
                {
                  echo '<script>window.location.href="panel.php?page=master-state";</script>';die();
                }
              }
              else
              {
            ...
            <div class="card card-primary">
				<div class="card-header">
					<h3 class="card-title">Edit <small>Country</small></h3>
				</div>
              <!-- /.card-header -->
              <!-- form start -->
				<form id="quickForm" method="POST">
					<div class="card-body">
						<div class="form-row">
							<div class="form-group col-md-6">
								<label>Country</label>
								<input type="text" name="name" class="form-control" id="country" placeholder="Enter Country" value="<?php echo $name; ?>">
							</div>
							<div class="form-group col-md-6">
								<label for="status">Status</label>
								<select id="status" name="status" class="form-control custom-select">
