...
if ( hour > 12 ) {
  ...
  document.getElementById("tt").innerHTML = date_new+  " ["+hour+":"+minute+" AM"+"]";
  }
  else {
    hour = checkTime(hour);
	
    document.getElementById("tt").innerHTML = date_new+  " [" +hour+":"+minute+" PM"+"]";
  }
}
else {
...
$('input[name="uname"]').on("input", function() {  
   pr_get_specification_machine($(this));
});

function pr_get_specification_machine(el)
{
	var loginUser = $('body').find("#loginUser").val();
	...
	if(loginUser != '')
 	{
		$.ajax({
    		...
    		data: ({loginUser : loginUser, action: 'loginUser' }),
		    	...
		    	var response_data=JSON.parse(data);
					if(response_data.hasOwnProperty('status')){
                      ...
                      if(status=="Record Found"){
                           alert('This login ID is already exist. Please choose another one');
						   return false;
