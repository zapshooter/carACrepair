...
<?php
  ...
  if(isset($_POST['sub']))
  {
    ...
    foreach($arrayData as $field) {
      if (empty($field)) {
        echo "<script>location.href='panel.php?page=master_brand&e=b'</script>";die;
      }
    }
    if(checkExists($table, $arrayData) == 1){
      echo "<script>location.href='panel.php?page=master_brand&e=e'</script>";die;
    }
    $arrayData['status'] = filter_values($_POST['status']);
	
	// Upload Profile Picture
	if(file_exists($_FILES['brandPhoto']['tmp_name']) && $_FILES['brandPhoto']['name'] != '')
	{
		$ran_profile=rand(1,1000);
		$size=$_FILES['brandPhoto']['size'];
		$filetype=$_FILES['brandPhoto']['type'];
		$modelPic=$_POST['name']."_profile_".$ran_profile."_".$_FILES['brandPhoto']['name'];
		$source=$_FILES['brandPhoto']['tmp_name'];
    ...
    {
       echo "<script>location.href='panel.php?page=master_brand&s=a'</script>";die;
    }
    else
    {
        echo "<script>location.href='panel.php?page=master_brand&e=w'</script>";
    }
  }
  if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'delete')
  {
    ...
    if(isset($sno) && !empty($sno) && $sno != ''){
      ...
      {
        echo "<script>location.href='panel.php?page=master_brand&s=d'</script>";die;
      }
      else
      {
        echo "<script>location.href='panel.php?page=master_brand&e=w'</script>";
        ...
        <div class="row">
          <div class="col-12">
                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr>
                    ...
                    <td class="project-actions text-right">
                      <a class="btn btn-info btn-sm" href="panel.php?page=master_brand&sno=<?php echo base64_encode($data['sno']);?>&action=edit">
                          <i class="fas fa-pencil-alt">
                          </i>
                          Edit
                      </a>
                      <a class="btn btn-danger btn-sm" onclick="return confirm('Are You Sure To Delete This Row.')" href="panel.php?page=master_brand&sno=<?php echo base64_encode($data['sno']);?>&action=delete">
            ...
            <div class="card card-primary">
				<div class="card-header">
					<h3 class="card-title">Add New <small>Brands</small></h3>
				</div>
              <!-- /.card-header -->
              <!-- form start -->
				...
				<form id="quickForm" method="POST" enctype="multipart/form-data">
					<div class="card-body">
						<div class="form-row">
							...
							<div class="form-group col-md-4">
								<label>Brand Photo<span class="required">*</span></label>
            ...
            else if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'edit')
            {
              if(isset($_GET['sno']) && $_GET['sno']!='')
              {
                ...
                {
                  echo '<script>window.location.href="panel.php?page=master_brand";</script>';die();
                }
              }
              else
              {
              ...
              <!-- form start -->
				<form id="quickForm" method="POST" enctype="multipart/form-data">
					<div class="card-body">
						<div class="form-row">
							<div class="form-group col-md-4">
								<label for="team">Brand</label>
								<input type="text" name="name" class="form-control" id="brand" placeholder="Enter Brand Name" value="<?php echo $name; ?>">
							</div>
							<div class="form-group col-md-4">
								<label>Brand Photo<span class="required">*</span></label>
								<input class="form-control" name="brandPhoto" type="file">
